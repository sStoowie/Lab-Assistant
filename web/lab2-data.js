const lab2Data = {
  ...
  steps: [
    ...
    { num: 9, task: "Task 1", desc: "ในส่วน DNS settings ให้ติ๊ก Enable DNS hostnames แล้วกด Save", errors: [
      { problem: "ลืมติ๊ก Enable DNS hostnames แล้วไปทำต่อ — จะไม่เห็นปัญหาจนถึงข้อ 72", cause: "DNS hostnames ต้อง enable เพื่อให้ EC2 instances ได้รับ Public DNS name — ถ้าไม่ทำตอนนี้ ข้อ 72 จะไม่มีค่า Public IPv4 DNS แสดง", fix: "ย้อนกลับ: VPC → Your VPCs → เลือก Lab VPC → Actions → Edit VPC settings → ติ๊ก Enable DNS hostnames → Save | หลัง enable อาจต้อง stop → start instance ให้ DNS name ปรากฏ" },
    ...
    { num: 48, task: "Task 6.2", desc: "ในส่วน Name and tags ใส่ Name=Public Instance (ต้องตรงตัว เป็น case sensitive)", errors: [
      { problem: "Session Manager connect ไม่ได้ในภายหลัง (ข้อ 80)", cause: "ชื่อ instance ไม่ตรง — lab ใช้ Name tag เพื่อ assign IAM permissions ผ่าน tag-based policy ดังนั้น ถ้าชื่อผิด (เช่น public instance, PublicInstance, Public_Instance) จะไม่มี permission", fix: "Name ต้องเป็น 'Public Instance' ตรงตัว: P ตัวใหญ่, I ตัวใหญ่, มีเว้นวรรค 1 ช่อง | ถ้าใส่ผิด ให้แก้ Name tag ที่ EC2 → Instances → เลือก instance → Tags tab → Edit" }
    ]},
    ...
    { num: 59, task: "Task 6.6", desc: "ตั้งค่า: VPC=Lab VPC, Subnet=Public Subnet, Auto-assign public IP=Enable", errors: [
    ...
    { num: 60, task: "Task 6.7", desc: "ที่ Firewall (security groups) เลือก Select existing security group", errors: [] },
    { num: 61, task: "Task 6.7", desc: "ใน Common security groups เลือก Public SG", errors: [
    ...
    { num: 62, task: "Task 6.9", desc: "กด expand ส่วน Advanced details", errors: [] },
    { num: 63, task: "Task 6.9", desc: "ที่ IAM instance profile เลือก EC2InstProfile จาก dropdown", errors: [
      ...
      { problem: "ข้ามขั้นตอนนี้ไป (ไม่ได้เลือก IAM role) → Session Manager connect ไม่ได้ (ข้อ 80)", cause: "IAM instance profile จำเป็นสำหรับ SSM Agent ในการ register กับ AWS Systems Manager", fix: "ถ้า launch ไปแล้วโดยไม่มี IAM role → terminate instance → launch ใหม่โดยเลือก EC2InstProfile ที่ Advanced details" }
    ]},
    { num: 64, task: "Task 6.9", desc: "ที่ User data ให้ paste script ที่ lab ให้มา (install Apache web server)", errors: [
      { problem: "Web server ไม่ทำงาน — ข้อ 74 เปิด web page ไม่ได้ (Connection Refused)", cause: "User data ไม่ได้ถูก execute — อาจ paste ไม่ครบ ไม่มี #!/bin/bash ข้างบน หรือ copy มาจาก Word/PDF ทำให้มี hidden characters", fix: "User data ต้องขึ้นต้นด้วย #!/bin/bash (บรรทัดแรก) | Copy จาก lab page โดยตรง ไม่ใช่จาก Word/PDF | ถ้า launch ไปแล้ว ใช้ Session Manager เข้าไป run script manually: sudo yum install -y httpd && sudo systemctl start httpd" },
    ...
    { num: 65, task: "Task 6.10", desc: "ดูส่วน Summary ตรวจว่าค่าถูกต้อง", errors: [] },
    { num: 66, task: "Task 6.10", desc: "กด Launch instance", errors: [
      ...
      { problem: "กด Launch แล้วเจอ error เรื่อง Network/Subnet", cause: "ไม่ได้กด Edit ที่ Network settings (ข้อ 58) หรือเลือก VPC/Subnet ผิด", fix: "ย้อนไปดูข้อ 58-59 ว่ากด Edit แล้วเลือก Lab VPC + Public Subnet + Enable public IP ครบ" },
      { problem: "กด Launch แล้วเจอ error เรื่อง Security Group", cause: "Security Group อยู่คนละ VPC กับ Subnet ที่เลือก", fix: "ตรวจว่า Network settings เลือก Lab VPC แล้ว SG ที่เลือกก็ต้องอยู่ใน Lab VPC เช่นกัน (ข้อ 61)" }
    ]},
    { num: 67, task: "Task 6.10", desc: "กด View all instances", errors: [] },
    { num: 68, task: "Task 6.10", desc: "รอจน Public Instance แสดง Instance state=Running และ Status check=2/2 checks passed", errors: [
    ...
    { num: 69, task: "Task 7", desc: "ที่เมนูด้านซ้าย เลือก Instances", errors: [] },
    { num: 70, task: "Task 7", desc: "เลือก (ติ๊ก) Public Instance", errors: [] },
    { num: 71, task: "Task 7", desc: "เลือก tab Networking ด้านล่าง", errors: [] },
    { num: 72, task: "Task 7", desc: "หาค่า Public IPv4 DNS", errors: [
      { problem: "ช่อง Public IPv4 DNS ว่างเปล่า ไม่มีค่าแสดง", cause: "VPC ไม่ได้ Enable DNS hostnames (ข้อ 9) หรือ instance ไม่ได้รับ public IP (ข้อ 17 หรือ 59)", fix: "ไป VPC → เลือก Lab VPC → Actions → Edit VPC settings → ติ๊ก Enable DNS hostnames → Save | และตรวจว่า instance มี public IP" },
    ...
    { num: 73, task: "Task 7", desc: "Copy ค่า Public IPv4 DNS (อย่ากด link 'open address' เพราะมันจะใช้ HTTPS)", errors: [] },
    { num: 74, task: "Task 7", desc: "เปิด browser tab ใหม่ วาง DNS ที่ copy มาโดยใช้ http:// ข้างหน้า ต้องเห็น web page ของ instance", errors: [
      { problem: "ERR_CONNECTION_TIMED_OUT หรือ web page ไม่แสดงเลย", cause: "มีหลายสาเหตุที่เป็นไปได้ ต้องเช็คทีละจุดตามลำดับ", fix: "เช็คตามลำดับ:\n1. Instance state เป็น Running และ 2/2 checks passed? (ข้อ 68)\n2. Instance มี Public IPv4 address?\n3. Security Group มี Inbound rule HTTP port 80? (ข้อ 42)\n4. Route table มี route 0.0.0.0/0 ไปหา Internet Gateway? (ข้อ 34)\n5. Internet Gateway ถูก Attach กับ Lab VPC? (ข้อ 25)\n6. Public Subnet ถูก associate กับ Public Route Table? (ข้อ 38)\n7. URL ใช้ http:// ไม่ใช่ https://?" },
      { problem: "ERR_CONNECTION_REFUSED", cause: "Web server (httpd) ไม่ได้ทำงาน เพราะ user data script ไม่ได้ถูกใส่หรือ run ไม่สำเร็จ", fix: "ใช้ Session Manager เข้าไปใน instance แล้ว run: sudo systemctl start httpd | ถ้าไม่เจอ httpd แสดงว่า user data ไม่ได้ใส่ (ข้อ 64) หรือเลือก AMI ผิด (ข้อ 52)" },
    ...
    { num: 75, task: "Task 7", desc: "ปิด browser tab กลับไป console", errors: [] },
    { num: 76, task: "Task 8", desc: "ค้นหา EC2 ใน search bar", errors: [] },
    { num: 77, task: "Task 8", desc: "ที่เมนูด้านซ้าย เลือก Instances", errors: [] },
    { num: 78, task: "Task 8", desc: "เลือก (ติ๊ก) Public Instance แล้วกด Connect", errors: [] },
    { num: 79, task: "Task 8", desc: "เลือก tab SSM Session Manager", errors: [] },
    { num: 80, task: "Task 8", desc: "กด Connect", errors: [
      { problem: "ปุ่ม Connect เป็นสีเทากดไม่ได้ หรือเจอ error 'We weren't able to connect to your instance'", cause: "SSM Agent ไม่สามารถ register กับ service ได้ มีหลายสาเหตุ", fix: "เช็คตามลำดับ:\n1. Name tag เป็น 'Public Instance' ตรงตัว? (ข้อ 48 — ต้อง P, I ตัวใหญ่ มีเว้นวรรค)\n2. IAM instance profile เป็น EC2InstProfile? (ข้อ 63)\n3. Instance state เป็น Running + 2/2? (ข้อ 68)\n4. รอ 3-5 นาทีหลัง launch แล้วหรือยัง?\n5. Instance มี internet access? (ข้อ 34, 25)" },
    ...
    { num: 81, task: "Task 8", desc: "พิมพ์คำสั่ง cd ~ แล้ว curl -I https://aws.amazon.com/training/", errors: [
      ...
      { problem: "curl ทำงานไม่ได้ทั้งที่ข้อ 74 web page เข้าได้ปกติ", cause: "อาจเป็นเพราะ Security Group outbound rule ถูกแก้ไข หรือ NACL block outbound traffic", fix: "เช็ค Security Group ของ instance ว่า Outbound rules มี All traffic allow | เช็ค Network ACL ว่าไม่ได้ block outbound" }
    ]},
    { num: 82, task: "Task 9", desc: "กลับไปที่ tab AWS Management Console", errors: [] },
    { num: 83, task: "Task 9", desc: "ค้นหา VPC ใน search bar", errors: [] },
    { num: 84, task: "Task 9", desc: "ที่เมนูด้านซ้าย เลือก NAT gateways", errors: [] },
    { num: 85, task: "Task 9", desc: "กด Create NAT gateway ตั้ง Name=Lab NGW, Subnet=Public Subnet, กด Allocate Elastic IP", errors: [
    ...
    { num: 86, task: "Task 9", desc: "กด Create NAT gateway จะเห็น success message", errors: [] },
    { num: 87, task: "Task 9", desc: "ที่เมนูด้านซ้าย เลือก Route tables", errors: [] },
    { num: 88, task: "Task 9", desc: "กด Create route table ตั้ง Name=Private Route Table, VPC=Lab VPC", errors: [
    ...
    { num: 89, task: "Task 9", desc: "กด Create route table จะเห็น success message", errors: [] },
    { num: 90, task: "Task 9", desc: "เลือก tab Routes", errors: [] },
    { num: 91, task: "Task 9", desc: "กด Edit routes", errors: [] },
    { num: 92, task: "Task 9", desc: "กด Add route ใส่ Destination=0.0.0.0/0 และ Target เลือก NAT Gateway แล้วเลือก Lab NGW", errors: [
    ...
    { num: 93, task: "Task 9", desc: "กด Save changes จะเห็น success message", errors: [] },
    { num: 94, task: "Task 9", desc: "เลือก tab Subnet associations", errors: [] },
    { num: 95, task: "Task 9", desc: "กด Edit subnet associations", errors: [] },
    { num: 96, task: "Task 9", desc: "ติ๊กเลือก Private Subnet แล้วกด Save associations", errors: [
      { problem: "ไม่เห็น Private Subnet ในลิสต์", cause: "Route table อยู่คนละ VPC กับ subnet", fix: "Delete route table นี้แล้วสร้างใหม่เลือก VPC=Lab VPC (ข้อ 88)" },
    ...
    { num: 97, task: "Task 9", desc: "กด Save associations จะเห็น success message", errors: [] },
    { num: 98, task: "Task 10", desc: "ที่เมนูด้านซ้าย เลือก Security Groups", errors: [] },
    { num: 99, task: "Task 10", desc: "กด Create security group ตั้ง Name=Private SG, VPC=Lab VPC", errors: [
    ...
    { num: 100, task: "Task 10", desc: "ในส่วน Inbound rules กด Add rule เลือก Type=HTTP, Source=Custom แล้วพิมพ์ sg ในช่อง จะเห็น Public SG ให้เลือก", errors: [
    ...
    { num: 101, task: "Task 10", desc: "ในส่วน Tags กด Add new tag ใส่ Key=Name, Value=Private SG", errors: [] },
    { num: 102, task: "Task 10", desc: "กด Create security group จะเห็น success message", errors: [] },
    { num: 103, task: "Task 11", desc: "ค้นหา EC2 ใน search bar", errors: [] },
    { num: 104, task: "Task 11.1", desc: "เลือก Dashboard จากเมนูด้านซ้าย", errors: [] },
    { num: 105, task: "Task 11.1", desc: "กด Launch instance", errors: [] },
    { num: 106, task: "Task 11.2", desc: "ในส่วน Name and tags", errors: [] },
    { num: 107, task: "Task 11.2", desc: "ใส่ Name=Private Instance (ต้องตรงตัว เป็น case sensitive)", errors: [] },
    { num: 108, task: "Task 11.3", desc: "ในส่วน Application and OS Images", errors: [] },
    { num: 109, task: "Task 11.3", desc: "ตรวจว่าเลือก Amazon Linux", errors: [] },
    { num: 110, task: "Task 11.3", desc: "ตรวจว่าเลือก Amazon Linux 2023 AMI", errors: [] },
    { num: 111, task: "Task 11.4", desc: "ในส่วน Instance type", errors: [] },
    { num: 112, task: "Task 11.4", desc: "เลือก t3.micro", errors: [] },
    { num: 113, task: "Task 11.5", desc: "ในส่วน Key pair", errors: [] },
    { num: 114, task: "Task 11.5", desc: "เลือก Proceed without a key pair", errors: [] },
    { num: 115, task: "Task 11.6", desc: "ในส่วน Network settings", errors: [] },
    { num: 116, task: "Task 11.6", desc: "กด Edit แล้วตั้งค่า: VPC=Lab VPC, Subnet=Private Subnet, Auto-assign public IP=Disable", errors: [
    ...
    { num: 117, task: "Task 11.7", desc: "ที่ Firewall เลือก Select existing security group", errors: [] },
    { num: 118, task: "Task 11.7", desc: "เลือก Private SG", errors: [
      { problem: "หา Private SG ไม่เจอใน dropdown", cause: "ตอนสร้าง Security Group (ข้อ 99) เลือก VPC ผิด (เป็น default VPC) หรือตอน launch ยังไม่ได้เลือก Lab VPC ใน Network settings (ข้อ 116)", fix: "ตรวจว่ากด Edit ที่ Network settings แล้วเลือก VPC=Lab VPC ก่อน แล้ว SG จะปรากฏ | ถ้า Private SG อยู่ผิด VPC ต้อง delete แล้วสร้างใหม่ใน Lab VPC (ข้อ 99) | หมายเหตุ: lab ไม่มี permission ให้ delete VPC ดังนั้นถ้าสร้าง SG ผิด VPC ต้อง delete SG แล้วสร้างใหม่ในLab VPC เท่านั้น" }
    ]},
    { num: 119, task: "Task 11.9", desc: "กด expand Advanced details", errors: [] },
    { num: 120, task: "Task 11.9", desc: "ที่ IAM instance profile เลือก EC2InstProfile", errors: [] },
    { num: 121, task: "Task 11.9", desc: "ที่ User data ให้ paste script เดียวกับตอน Public Instance", errors: [] },
    { num: 122, task: "Task 11.11", desc: "ดูส่วน Summary ตรวจความถูกต้อง", errors: [] },
    { num: 123, task: "Task 11.11", desc: "กด Launch instance", errors: [
      { problem: "'You are not authorized' หรือ 'Instance limit exceeded'", cause: "Instance type ไม่ใช่ t3.micro, เลือก AMI ผิด, หรือมี instance รันอยู่เกิน limit ของ lab", fix: "เช็ค: instance type เป็น t3.micro? (ข้อ 112) | AMI เป็น Amazon Linux 2023? (ข้อ 110) | terminate instance เก่าที่ไม่ได้ใช้" },
      { problem: "กด Launch แล้วเจอ error เรื่อง Network/Subnet", cause: "ไม่ได้กด Edit ที่ Network settings (ข้อ 116) หรือเลือก VPC/Subnet ผิด", fix: "ย้อนไปดูข้อ 116 ว่ากด Edit แล้วเลือก Lab VPC + Private Subnet + Disable public IP ครบ" },
      { problem: "กด Launch แล้วเจอ error เรื่อง Security Group", cause: "Security Group อยู่คนละ VPC กับ Subnet ที่เลือก", fix: "ตรวจว่า Network settings เลือก Lab VPC แล้ว SG ที่เลือกก็ต้องอยู่ใน Lab VPC เช่นกัน (ข้อ 118)" }
    ]},
    { num: 124, task: "Task 11.11", desc: "กด View all instances", errors: [] },
    { num: 125, task: "Task 11.11", desc: "รอจน Private Instance แสดง Running + 2/2 checks passed", errors: [
    ...
    { num: 126, task: "Task 12", desc: "ที่เมนูด้านซ้าย เลือก Instances", errors: [] },
    { num: 127, task: "Task 12", desc: "เลือก (ติ๊ก) Private Instance แล้วกด Connect", errors: [] },
    { num: 128, task: "Task 12", desc: "เลือก tab SSM Session Manager", errors: [] },
    { num: 129, task: "Task 12", desc: "กด Connect", errors: [
      { problem: "ปุ่ม Connect เป็นสีเทากดไม่ได้ หรือ connect ไม่สำเร็จ", cause: "เหมือนข้อ 80 แต่สำหรับ Private Instance ที่ต้อง route ผ่าน NAT", fix: "เช็คตามลำดับ:\n1. Name tag เป็น 'Private Instance' ตรงตัว? (ข้อ 107)\n2. IAM instance profile เป็น EC2InstProfile? (ข้อ 120)\n3. Instance state เป็น Running + 2/2? (ข้อ 125)\n4. Private Route Table มี route 0.0.0.0/0 ไปหา NAT Gateway? (ข้อ 92)\n5. NAT Gateway อยู่ใน Public Subnet และ state=Available? (ข้อ 85)\n6. รอ 3-5 นาทีหลัง launch" },
      ...
      { problem: "NAT Gateway state เป็น Available แต่ private instance ยังไม่มี internet", cause: "NAT Gateway อยู่ใน Public Subnet แต่ route 0.0.0.0/0 ใน Private Route Table ชี้ไป NAT ผิดตัว หรือ Private Subnet ไม่ได้ associate กับ Private Route Table (ข้อ 96)", fix: "เช็ค: Private RT associate กับ Private Subnet? (ข้อ 96) | Route 0.0.0.0/0 ชี้ไป NAT ตัวที่ถูก? (ข้อ 92)" }
    ]},
    { num: 130, task: "Task 12", desc: "พิมพ์คำสั่ง cd ~ แล้ว curl -I https://aws.amazon.com/training/", errors: [
      { problem: "curl: (28) Connection timed out", cause: "Private Instance ไม่มีทางออก internet ผ่าน NAT Gateway", fix: "เช็คตามลำดับ:\n1. Private Route Table มี route 0.0.0.0/0 ไปหา NAT Gateway? (ข้อ 92)\n2. Private Subnet ถูก associate กับ Private Route Table? (ข้อ 96)\n3. NAT Gateway อยู่ใน Public Subnet? (ข้อ 85)\n4. NAT Gateway state เป็น Available?\n5. Public Route Table มี route 0.0.0.0/0 ไปหา Internet Gateway? (ข้อ 34)\n6. Internet Gateway ถูก Attach กับ Lab VPC? (ข้อ 25)" },
      { problem: "curl ได้ผลเป็น HTTP/2 200 (สำเร็จ)", cause: "ถูกต้องแล้ว นี่คือ expected output", fix: "ไม่ต้องทำอะไร ข้อนี้ pass" }
    ]},
    { num: 131, task: "Task 12", desc: "ปิด Session Manager tab กลับไป console", errors: [] },
    { num: 132, task: "Optional 1", desc: "กลับไป AWS Management Console", errors: [] },
    { num: 133, task: "Optional 1", desc: "ที่เมนูด้านซ้าย เลือก Instances", errors: [] },
    { num: 134, task: "Optional 1", desc: "เลือก (ติ๊ก) Private Instance", errors: [] },
    { num: 135, task: "Optional 1", desc: "ที่ tab Details ให้ copy ค่า Private IPv4 addresses", errors: [] },
    { num: 136, task: "Optional 1", desc: "เอาติ๊ก Private Instance ออก", errors: [] },
    { num: 137, task: "Optional 1", desc: "เลือก (ติ๊ก) Public Instance", errors: [] },
    { num: 138, task: "Optional 1", desc: "กด Connect", errors: [] },
    { num: 139, task: "Optional 1", desc: "เลือก tab Session Manager", errors: [] },
    { num: 140, task: "Optional 1", desc: "กด Connect เพื่อเข้า Public Instance", errors: [] },
    { num: 141, task: "Optional 1", desc: "พิมพ์ curl แล้วตามด้วย Private IP ที่ copy มา (เช่น curl 10.0.2.131)", errors: [
      { problem: "curl timeout หรือ Connection refused", cause: "Private Security Group ไม่มี inbound rule HTTP ที่ source เป็น Public SG หรือ httpd ไม่ได้ running บน Private Instance", fix: "ตรวจ Private SG ว่ามี inbound HTTP โดย source เป็น Public SG (ข้อ 100) | ถ้า httpd ไม่ running ให้ SSM เข้า Private Instance แล้ว run: sudo systemctl start httpd" }
    ]},
    { num: 142, task: "Optional 1", desc: "พิมพ์ ping แล้วตามด้วย Private IP (เช่น ping 10.0.2.131)", errors: [
    ...
    { num: 143, task: "Optional 1", desc: "copy คำสั่ง ping ที่ lab ให้", errors: [] },
    { num: 144, task: "Optional 1", desc: "กด CTRL+C เพื่อหยุด ping", errors: [] },
    { num: 145, task: "Optional 2", desc: "กลับไปที่ tab AWS Management Console", errors: [] },
    { num: 146, task: "Optional 2", desc: "ที่เมนูด้านซ้าย เลือก Instances", errors: [] },
    { num: 147, task: "Optional 2", desc: "เลือก (ติ๊ก) Public Instance", errors: [] },
    { num: 148, task: "Optional 2", desc: "กด Connect", errors: [] },
    { num: 149, task: "Optional 2", desc: "เลือก tab Session Manager", errors: [] },
    { num: 150, task: "Optional 2", desc: "กด Connect", errors: [] },
    { num: 151, task: "Optional 2", desc: "run คำสั่ง IMDSv2 token แล้ว curl metadata", errors: [
    ...
    { num: 152, task: "Optional 2", desc: "run คำสั่ง curl เพื่อดู public-hostname ของ instance", errors: [] },
    { num: 153, task: "End Lab", desc: "กลับไป AWS Management Console", errors: [] },
    { num: 154, task: "End Lab", desc: "ที่มุมบนขวา กด AWSLabsUser แล้วกด Sign out", errors: [] },
    { num: 155, task: "End Lab", desc: "กด End Lab แล้ว confirm เพื่อจบ lab", errors: [] }
  ]
};
